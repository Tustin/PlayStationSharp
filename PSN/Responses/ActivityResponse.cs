...
namespace PSN.Responses
{
    public class Activity
    {
        ...
        public class Feed
        {
            ...
            public async Task<bool> LeaveComment(string comment) {
                var c = new
                {
                    commentString = comment
                };

                var response = await Utilities.SendJsonPostRequest($"https://activity.api.np.km.playstation.net/activity/api/v1/users/me/comment/{this.storyId}", c, Auth.CurrentInstance.AccountTokens.Authorization).ReceiveJson();
            ...
            public async Task<bool> Like() {
                var response = await Utilities.SendJsonPostRequest($"https://activity.api.np.km.playstation.net/activity/api/v1/users/{Auth.CurrentInstance.Profile.onlineId}/set/like/story/{this.storyId}", null, Auth.CurrentInstance.AccountTokens.Authorization).ReceiveJson();
            ...
            public async Task<bool> Unlike() {
                var response = await Utilities.SendJsonPostRequest($"https://activity.api.np.km.playstation.net/activity/api/v1/users/{Auth.CurrentInstance.Profile.onlineId}/set/dislike/story/{storyId}", null, Auth.CurrentInstance.AccountTokens.Authorization).ReceiveJson();
